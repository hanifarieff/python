...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/Staging/logs/LogTransAppointmentQueueOld.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT * FROM
                                ...
                                WHERE
                                -- (a.admission_dttm >= '2023-05-05 00:00:00' AND a.admission_dttm <= '2023-05-05 23:59:59') 
                                ...
                                -- AND new.AppointmentID IN ('196946-178','13492791')
...
# try:
#     source.to_sql('TransAppointmentQueueOld',schema='staging_rscm',con=conn_staging_sqlserver, if_exists='append',index=False)
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f'SELECT AppointmentID,AdmissionDate,PatientID,AdmissionID,PatientType,MedicalNo,BPJSNo,SEPNo,NIK,PhoneNo,PoliCode,PoliName, NewPatient,AppointmentDate,DoctorCode,DoctorName,AppointmentTime,VisitType,AppointmentMethod,ReferenceNo,QueueNumber,QueueNo,ServedEstimated,RestQuotaJKN,QuotaJKN,RestQuotaNonJKN,QuotaNonJKN,Notes FROM staging_rscm.TransAppointmentQueueOld where AppointmentID IN ({appointmentid}) AND PatientID IN ({patientid}) AND AdmissionID IN ({admissionid}) order by AppointmentID'
        ...
        admissionid = tuple(source["AdmissionID"].unique())
         ...
         # query buat narik data dari target lalu filter berdasarkan primary key
        query = f'SELECT AppointmentID,AdmissionDate,PatientID,AdmissionID,PatientType,MedicalNo,BPJSNo,SEPNo,NIK,PhoneNo,PoliCode,PoliName, NewPatient,AppointmentDate,CAST(DoctorCode as VARCHAR(10)) AS DoctorCode,DoctorName,AppointmentTime,VisitType,AppointmentMethod,ReferenceNo,QueueNumber,QueueNo,ServedEstimated,RestQuotaJKN,QuotaJKN,RestQuotaNonJKN,QuotaNonJKN,Notes FROM staging_rscm.TransAppointmentQueueOld where AppointmentID IN {appointmentid} AND PatientID IN {patientid} AND AdmissionID IN {admissionid} order by AppointmentID'
        target = pd.read_sql_query(query, conn_staging_sqlserver)
    ...
    modified = changes[changes[['AppointmentID','PatientID','AdmissionID']].apply(tuple,1).isin(target[['AppointmentID','PatientID','AdmissionID']].apply(tuple,1))]
    ...
    inserted = changes[~changes[['AppointmentID','PatientID','AdmissionID']].apply(tuple,1).isin(target[['AppointmentID','PatientID','AdmissionID']].apply(tuple,1))]
    ...
    if modified.empty:
        ...
        inserted.to_sql('TransAppointmentQueueOld', schema='staging_rscm', con = conn_staging_sqlserver, if_exists = 'append', index=False)
        ...
        try:
            # update data
            updated_to_sql(modified, 'TransAppointmentQueueOld', 'AppointmentID','PatientID','AdmissionID')
