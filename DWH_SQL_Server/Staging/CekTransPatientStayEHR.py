...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/Staging/logs/LogCekTransStayIndEHR.txt","w")
t0 = time.time()
...
if source.empty:
    ...
    if len(source) == 1:
        ...
        query = f'SELECT PatientID,AdmissionID,MedicalNo,AdmissionDate,StayInd from staging_rscm.TransPatientStay where PatientID IN ({patientid}) AND AdmissionID IN ({admissionid}) AND Flag = 1 order by PatientID, AdmissionID'
