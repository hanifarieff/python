...
if source.empty:
    ...
    query = f'SELECT PatientID,AdmissionID,MedicalNo,AdmissionDate,StayInd,StayIndAdmission,Flag from staging_rscm.TransPatientStay where PatientID IN {patientid} AND AdmissionID IN {admissionid} AND Flag = 1 order by PatientID, AdmissionID'
