...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/Staging/logs/LogTransPatientOrderRoleHIS.txt","w")
t0 = time.time()
...
source_order = pd.read_sql_query("""
                               SELECT 
                                ...
                                -- ((CAST(CreatedDate as date) >= CAST(DATEADD(DAY, -2, GETDATE()) as date) AND CAST(CreatedDate as date) <= CAST(GETDATE() as date))
                                -- OR
...
source_order['OrderID'] = source_order['OrderID'].astype('int64') 

query_order_role = f"""SELECT order_id as OrderID, role_no as RoleNo, clin_priv_id as DefaultObjID, CAST(tariff as decimal(15,2)) as TarifAmount, CAST(discount as decimal(15,2)) as Discount
                        FROM xocp_his_patient_order_role where order_id IN {orderid} """
source_order_role = pd.read_sql_query(query_order_role, conn_his)

source = source_order.merge(source_order_role, how='left',on=['OrderID'])
...
source['DefaultObjID'] = source['DefaultObjID'].fillna('').astype('str').str.replace('.0', '')
source['DefaultObjID'] = source['DefaultObjID'].replace('', None)
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TRIM(OrderID) AS OrderID,RoleNo,DefaultObjID,TarifAmount,Discount,OrderDate,CreatedDate,Flag FROM staging_rscm.TransPatientOrderRole where OrderID IN ({orderid}) AND RoleNo IN ({roleno}) AND Flag = 2 ORDER BY OrderID, RoleNo"
