...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/Staging/logs/LogTransFormPathologyAnatomy.txt","w")
...
source = pd.read_sql_query(""" 
                            SELECT  
                                ...
                                CASE
                                    WHEN lab.sub_histo = 'htk' THEN 'Histokimia'
                                    WHEN lab.sub_histo = 'his' THEN 'Histopatologi'
                                    ELSE '-'
                                END AS Type,
                                CASE
                                ...
                                -- MRN ambil ke MPI
                                lab.cara_pengambilan TissueSource, -- asal jaringan,
                                lab.diag_klin as DiagnosticHistory,
                                NULL as PADiagnose -- DiagnosisPA hanya untuk imuno dan patmol
                               
                                -- DPJP sama dengan SPV
                                -- Join ke FactPathologyAnatomy ambil Keterangan 
                            FROM xocp_his_pa_form_histopatologi lab
                            ...
                            where 
                            lab.created_dttm >= '2024-10-31 00:00:00' and lab.created_dttm <= '2024-10-31 23:59:59'
                            -- and lab.no_pa_id = 'LH24-0339'
                            -- lab.patient_id = 1054602 and lab.admission_id = 10
                            -- and adm.payplan_id NOT IN('8','71')
                            -- lab.no_pa_id IN ('H24-09506','H24-09521','H24-08177','H24-07886','LH24-0345')
                            GROUP BY 
                                lab.patient_id,
                                lab.admission_id,
                                lab.form_number,
...
if source.empty:
    ...
    patientid = remove_comma(patient_id)
    admissionid = remove_comma(admission_id)
