...
query = f""" select 
                distinct 
                    ...
                    pat.patient_mrn_txt as MedicalNo,
                ...
                -- and
                (ps.created_dttm >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), "%%Y-%%m-%%d 00:00:00") 
...
dummy = pd.read_sql_query(""" SELECT MedicalNo FROM staging_rscm.DimensionDummyPatient """, conn_staging_sqlserver)
...
source = source.groupby(['ExtractDate','CreatedDate','FlagDB','Sensus'],dropna=False).size().reset_index(name='RowTotal')
...
if source.empty:
    ...
    query = f'SELECT ExtractDate, CreatedDate, FlagDB, Sensus, RowTotal from dwhrscm_talend.LogPatientStay where ExtractDate IN {extract_date} AND CreatedDate IN {created_date} AND FlagDB IN {flag_db} AND Sensus IN {sensus} order by ExtractDate,CreatedDate,FlagDB,Sensus'
    ...
    modified = changes[changes[['ExtractDate','CreatedDate','FlagDB','Sensus']].apply(tuple,1).isin(target[['ExtractDate','CreatedDate','FlagDB','Sensus']].apply(tuple,1))]
    ...
    inserted = changes[~changes[['ExtractDate','CreatedDate','FlagDB','Sensus']].apply(tuple,1).isin(target[['ExtractDate','CreatedDate','FlagDB','Sensus']].apply(tuple,1))]
    ...
    if modified.empty:
        ...
        try:
            # update data
            updated_to_sql(modified, 'LogPatientStay', 'ExtractDate', 'CreatedDate', 'FlagDB','Sensus')
