...
query_source3 = f""" SELECT DISTINCT
                                ...
                                x.updated_dttm >= {start_date} AND x.updated_dttm <= {end_date}
                                 """
source3= pd.read_sql_query(query_source3,conn_his_live)
print(source3)
...
if source.empty:
    ...
    query = f'SELECT PatientID,AdmissionID,MedicalNo,AdmissionDate,StayInd,StayIndAdmission,Flag from staging_rscm.TransPatientStay where PatientID IN {patientid} AND AdmissionID IN {admissionid} AND Flag = 2 order by PatientID, AdmissionID'
