...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogCekFactStayIndHIS.txt","w")
t0 = time.time()
...
if source.empty:
    ...
    if len(source) == 1:
        ...
        query = f'SELECT PatientID,AdmissionID,MedicalNo,AdmissionDate,StayInd from dwhrscm_talend.FactPatientStay where PatientID IN ({patientid}) AND AdmissionID IN ({admissionid}) AND Flag = 2 order by PatientID, AdmissionID'
