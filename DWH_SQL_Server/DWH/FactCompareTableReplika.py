...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogFactCompareTableReplika.txt","w")
t0 = time.time()
...
# connection database
conn_ehr_replika = db_connection.create_connection(db_connection.replika_ehr)
conn_his_replika = db_connection.create_connection(db_connection.replika_his)
...
def get_data_ehr_live(*list_table):
    ...
    for index, val in enumerate(list_table, start=1):
        try:
            ...
            data = pd.read_sql_query(query, conn_ehr_replika)
            ...
            print(e)
    source_ehr = pd.concat(dataframes, ignore_index=True)
    return source_ehr
    print('\n')
...
def get_data_his_live(*list_table):
    ...
    for index, val in enumerate(list_table, start=1):
        try:
            query = (f"SELECT COUNT(*) AS RowTotal FROM {val}")
            data = pd.read_sql_query(query, conn_his_replika)
            data.insert(0,'ID',index)
            data.insert(1,'TableName',val)
            dataframes.append(data)
        except Exception as e:
...
ehr = get_data_ehr_live('xocp_ehr_patient_admission','xocp_ehr_patient_stay','xocp_orgs','xocp_ehr_patient_act','xocp_ehr_patient','xocp_ehr_payplan','xocp_ehr_company','xocp_ehr_patient_order')
his = get_data_his_live('xocp_his_patient_admission','xocp_his_patient_location','xocp_orgs','xocp_his_patient_act','xocp_his_patient','xocp_his_obj_payplan','xocp_his_formulir_darah','xocp_his_patient_order')

# gabungkan variabel ehr dan his jadi 1 dataframe
source = pd.concat([ehr, his], ignore_index=True)
source['ID'] = source.index + 1 # Setting 'ID' as 1-based index
print(source)

id = tuple(source['ID'])

query = f"SELECT ID, TableName, RowTotal FROM dwhrscm_talend.FactRowNumberReplika WHERE ID IN {id}"
...
if modified.empty:
    ...
    inserted.to_sql('FactRowNumberReplika', schema = 'dwhrscm_talend',con = conn_dwh_sqlserver, if_exists = 'append', index=False)
    ...
    try:
        # update data
        updated_to_sql(modified, 'FactRowNumberReplika', 'ID')
...
db_connection.close_connection(conn_ehr_replika)
