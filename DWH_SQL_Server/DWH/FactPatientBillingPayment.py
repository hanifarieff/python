...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogFactPatientBillingPayment.txt","w")
t0 = time.time()
...
source = pd.read_sql_query("""
                           SELECT
                                ...
                                a.TransactionDate,
                                a.Status
                            FROM staging_rscm.TransPatientBillingPayment a 
                            WHERE a.MedicalNo NOT IN (SELECT MedicalNo FROM staging_rscm.DimensionDummyPatient)
...
if source.empty:
    ...
    if len(source) == 1:    
        ...
        BillingTransactID = source["BillingTransactID"].values[0]     

        # query buat narik data dari target lalu filter berdasarkan primary key
        query = f'SELECT BillingTransactID, CompanyName, PatientName, MedicalNo, AdmissionDate, TransactionDate,Status FROM dwhrscm_talend.FactPatientBillingPayment WHERE BillingTransactID IN ({BillingTransactID})'
    ...
    if modified.empty:
        if inserted.empty :
            ...
            inserted.to_sql('FactPatientBillingPayment', schema='dwhrscm_talend' ,con=conn_dwh_sqlserver, if_exists = 'append', index=False)
        ...
        try:
            #update data
            updated_data(modified, 'FactPatientBillingPayment', 'BillingTransactID')
