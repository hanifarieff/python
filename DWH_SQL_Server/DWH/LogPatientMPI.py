...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogRowPatientMPI.txt","w")
...
query = f""" SELECT 
            ...
            DATE(created_dttm) as CreatedDate,
            COUNT(*) TotalPatientMPI
            FROM patients
            WHERE created_dttm >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), "%%Y-%%m-%%d 00:00:00") 
                            and created_dttm <= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), "%%Y-%%m-%%d 23:59:59")
            """
source = pd.read_sql_query(query, conn_mpi)
...
query_dwh = f"""
            ...
            COUNT(*) as TotalPatientDWH
            FROM dwhrscm_talend.DimPatientMPI where ScdActive = '1'
            AND CAST(PatientCreatedDate as date) >=  CAST(DATEADD(DAY, -1, GETDATE()) as date) AND CAST(PatientCreatedDate as date) <= CAST(GETDATE() as date)
...
if source['TotalPatientMPI'][0] != source['TotalPatientDWH'][0]:
    # Email Details
    ...
    subject = "Alert: Data Table MPI Mismatch"
    body = (
        f"Dear Hanif,\n\n"
        f"A mismatch has been detected in the patient data:\n"
        f"TotalPatientMPI: {source['TotalPatientMPI'][0]}\n"
        f"TotalPatientDWH: {source['TotalPatientDWH'][0]}\n\n"
...
if source.empty:
    ...
    query = f'SELECT ExtractDate, CreatedDate,TotalPatientMPI, TotalPatientDWH from dwhrscm_talend.LogPatientMPI where ExtractDate IN {extract_date} order by ExtractDate'
    ...
    target['CreatedDate'] = pd.to_datetime(target.CreatedDate, format='%Y-%m-%d')
    ...
    if modified.empty:
        if inserted.empty:
            ...
            inserted.to_sql('LogPatientMPI', schema='dwhrscm_talend', con = conn_dwh_sqlserver, if_exists = 'append', index=False)
        ...
        try:
            # update data
            updated_to_sql(modified, 'LogPatientMPI', 'ExtractDate')
