...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogFactCompareTableMasterEHR.txt","w")
t0 = time.time()
...
def get_data_ehr_live(*list_table):
    ...
    for index, val in enumerate(list_table, start=1):
        try:
            ...
            data = pd.read_sql_query(query, conn_ehr_live)
...
source = get_data_ehr_live('xocp_ehr_patient_admission','xocp_ehr_patient_stay','xocp_orgs','xocp_ehr_patient_act','xocp_ehr_patient','xocp_ehr_payplan','xocp_ehr_company','xocp_ehr_patient_order')
# his = get_data_his_live('xocp_his_patient_admission','xocp_his_patient_location','xocp_orgs')
