""" File ini berfungsi untuk menarik data dari tabel `TransResultFormUrology` ke `DWH` """
...
# log path file error
log_file_path = "C:/TestPython/DWH_SQL_Server/DWH/Log/FactResultFormUrology.log"
...
def get_source_data(conn_staging_sqlserver):
    ...
    source = pd.read_sql_query("""                                  
                                SELECT 
                                    ...
                                    a.StatusForm,
                                    a.CreatedUserID
                                FROM staging_rscm.TransResultFormUrology a
...
def get_target_data(source, conn_dwh_sqlserver):
    ...
    query = f"""SELECT     
                    ...
                    StatusForm,
                    CreatedUserID
                FROM dwhrscm_talend.FactResultFormUrology WHERE OrderID IN {order_id} AND PatientID IN {patient_id} 
                AND AdmissionID IN {admission_id} AND ObjID IN {obj_id}
            """
...
def inserted_data(inserted, conn_dwh_sqlserver):
    ...
    if not inserted.empty:
        with conn_dwh_sqlserver.begin() as transaction:
            ...
            inserted.to_sql('FactResultFormUrology', schema='dwhrscm_talend', con=conn_dwh_sqlserver, if_exists='append', index=False)
...
def main():
    ...
    try:
        ...
        if source.empty:
            ...
            # update data
            updated_data(modified, 'FactResultFormUrology', 'OrderID','PatientID','AdmissionID','ObjID','SequenceID', conn_dwh_sqlserver)
        ...
        error_message = (
            f"Dear Hanif,\n\n"
            f"There is an error in your script to FactResultFormUrology, here's the error :\n\n"
            f"{str(e)}\n\n"
        ...
        send_email(subject='Alert For FactResultFormUrology',body=error_message)
