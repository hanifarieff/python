...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogFactOrderBasedEHR.txt","w")
t0 = time.time()
...
def get_connections():
    ...
    return conn_ehr, conn_dwh_sqlserver

def get_source_data(conn_ehr, conn_dwh_sqlserver, date_range):
    ...
    query_source = f"""                            
                    SELECT 
                        ...
                        (
                            ...
                            WHERE
                            a.ordered_dttm >= '{date_range} 00:00:00' AND a.ordered_dttm <= '{date_range} 23:59:59'
                            ...
                            -- and a.order_id IN ('00210005722035')	
                            GROUP BY 
                                a.order_id,
                                a.patient_id,
                                a.admission_id,
...
def get_target_data(source, conn_dwh_sqlserver):
    ...
    query_filter = f"""SELECT OrderID,PatientID,AdmissionID,EmployeeID,DoctorName,KSM,NIP,NIK,PatientName,MedicalNo,SEPNo,OrgName,AdmissionDate,
                        ...
                        JasaRemun,PayplanName,CategoryName,PayplanKemkes,Type,Status,ObjectGroupingName
...
def main():
    ...
    conn_ehr, conn_dwh_sqlserver = get_connections()

    try:
        # Ambil tanggal dan jam hari ini
        today = datetime.now()
        day_of_today = today.strftime('%d')
        
        # Ambil tanggal pertama untuk bulan ini
        first_day_of_current_month = today.replace(day=1)

        # Ambil tanggal terakhir untuk bulan sebelumnya
        last_day_of_previous_month = first_day_of_current_month - timedelta(days=1)

        # jika sekarang tanggal 01, maka batas tanggal terakhir adalah 5, jika tanggal 2 maka tanggal 10, dan seterusnya selisih 5 hari
        if day_of_today == '01':
            # define tanggal menjadi tanggal 5
            end_of_day = last_day_of_previous_month.replace(day=5)
            # ambil angka tanggalnya saja
            day = end_of_day.day
        elif day_of_today == '02':
            # define tanggal menjadi tanggal 10
            end_of_day = last_day_of_previous_month.replace(day=10)
            # ambil angka tanggalnya saja
            day = end_of_day.day
        elif day_of_today == '03':
            # define tanggal menjadi tanggal 15
            end_of_day = last_day_of_previous_month.replace(day=15)
            # ambil angka tanggalnya saja
            day = end_of_day.day
        elif day_of_today == '04':
            # define tanggal menjadi tanggal 20
            end_of_day = last_day_of_previous_month.replace(day=20)
            # ambil angka tanggalnya saja
            day = end_of_day.day
        elif day_of_today == '05':
            # define tanggal menjadi tanggal 25
            end_of_day = last_day_of_previous_month.replace(day=25)
            # ambil angka tanggalnya saja
            day = end_of_day.day
        else:
            # tanggal terakhir bulan sebelumnya
            end_of_day = last_day_of_previous_month
            day = end_of_day.day

        # buat range tanggal diambil dari variable end_of_day, misal end_of_day berarti range tanggal 1-5, end_of_day 10 rangenya tanggal 6-10  
        for x in range(day-4, day+1):
            date_range = end_of_day.replace(day=x).date()
            print('ini range tanggal')
            print(date_range)

            # Ambil data dari source
            source = get_source_data(conn_ehr,conn_dwh_sqlserver,date_range)
            print("Source Data:")
