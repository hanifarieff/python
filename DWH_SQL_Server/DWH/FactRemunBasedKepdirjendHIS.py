...
sys.stdout = open("C:/TestPython/DWH_SQL_Server/DWH/logs/LogFactRemunBasedHIS.txt","w")
t0 = time.time()
...
query_source = f""" SELECT 
                        ...
                        CASE
                            WHEN b.tarif_transact IS NULL THEN NULL
                            ELSE CAST(b.tarif_transact as DECIMAL(15,2))
                        END as Tarif,
                        ...
                        -- dm.order_no as RoleNo -- buat join ke his_patient_order_role
...
if not filtered_order_id.empty:
    ...
    query_patient_order = f"""
        SELECT 
				...
				a.obj_id as ObjectIDNull,
				b.tariff as TarifNew,
				org.org_nm OrgNameNew
        FROM xocp_his_patient_order a
        LEFT JOIN xocp_his_obj_payplan b on a.obj_id = b.obj_id and a.payplan_id = b.payplan_id
        -- LEFT JOIN xocp_his_patient_admission adm on adm.patient_id = a.patient_id and adm.admission_id = a.admission_id
        LEFT JOIN xocp_orgs org on org.org_id = a.org_id
        WHERE a.order_id IN {order_id_tuple}"""
    ...
    source['Tarif']=source['TarifNew']
    source['OrgName'] = source['OrgNameNew']
else:
...
try:
    with conn_dwh_sqlserver.begin() as transaction:
        chunksize_size = 1000
        source.to_sql('FactRemunBasedNew', schema='dwhrscm_talend',con=conn_dwh_sqlserver, if_exists ='append',index=False,chunksize=chunksize_size)
