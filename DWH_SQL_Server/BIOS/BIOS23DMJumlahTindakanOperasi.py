...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS23DMJumlahTindakanOperasi.txt","w")
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[23DMJumlahTindakanOperasi]
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS23DMJumlahTindakanOperasi WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS23DMJumlahTindakanOperasi', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
