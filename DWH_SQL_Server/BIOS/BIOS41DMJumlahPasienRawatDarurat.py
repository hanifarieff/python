...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS41DMJumlahPasienRawatDarurat.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[108DMJumlahPasienRawatDarurat]
                                WHERE
                                -- TglTransaksi >= '2024-02-11 00:00:00' AND TglTransaksi <= '2024-02-20 23:59:59'
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS41DMJumlahPasienRawatDarurat WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS41DMJumlahPasienRawatDarurat', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
