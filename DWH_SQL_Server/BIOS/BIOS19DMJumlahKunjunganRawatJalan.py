...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS19DMJumlahKunjunganRawatJalan.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[19DMJumlahKunjunganRawatJalan]
                                WHERE
                                -- TglTransaksi >= '2024-02-16 00:00:00' AND TglTransaksi <= '2024-02-19 23:59:59' 
                                TglTransaksi >= CAST(DATEADD(DAY, -5, GETDATE()) as date)
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS19DMJumlahKunjunganRawatJalan WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS19DMJumlahKunjunganRawatJalan', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
        ...
        try:
            #update data
            updated_to_sql(modified, 'BIOS19DMJumlahKunjunganRawatJalan', 'TglTransaksi')
