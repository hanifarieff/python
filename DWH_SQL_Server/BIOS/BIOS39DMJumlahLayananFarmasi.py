...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/Log39DMJumlahLayananFarmasi.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.GetJumlahLayananFarmasi(
                                CAST(DATEADD(DAY, -5, GETDATE()) as date),CAST(GETDATE() as date))
                             """, conn_dwh_sqlserver)
...
query = f'SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS39DMJumlahLayananFarmasi WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent']=0
    inserted.to_sql('BIOS39DMJumlahLayananFarmasi',schema='dwhrscm_talend', con = conn_bios_sqlserver, if_exists = 'append', index=False)
