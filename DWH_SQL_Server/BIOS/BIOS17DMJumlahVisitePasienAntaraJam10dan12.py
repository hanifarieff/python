...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS17DMJumlahVisitePasienAntaraJam10Dan12.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[17DMJumlahVisitePasienAntaraJam10Dan12]
                                WHERE TglTransaksi >= CAST(DATEADD(DAY, -10, GETDATE()) as date)
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS17DMJumlahVisitePasienAntaraJam10Dan12 WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS17DMJumlahVisitePasienAntaraJam10Dan12', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
        ...
        try:
            #update data
            updated_to_sql(modified, 'BIOS17DMJumlahVisitePasienAntaraJam10Dan12', 'TglTransaksi')
