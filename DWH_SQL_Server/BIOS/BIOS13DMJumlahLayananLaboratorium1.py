...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS13DMJumlahLayananLaboratorium1.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[13DMJumlahLayananLaboratorium1]
                                WHERE 
                                TglTransaksi >= CAST(DATEADD(DAY, -10, GETDATE()) as date)
                                AND TglTransaksi <= CAST(GETDATE() as date)
                                -- TglTransaksi >= '2023-05-18' AND TglTransaksi <= '2023-05-18'
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS13DMJumlahLayananLaboratorium1 WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS13DMJumlahLayananLaboratorium1', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
