...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS16DMJumlahVisitePasienDiBawahJam10.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    TglTransaksi,
                                    Jumlah 
                                FROM dwhrscm_talend.[16DMJumlahVisitePasienDiBawahJam10]
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS16DMJumlahVisitePasienDiBawahJam10 WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS16DMJumlahVisitePasienDiBawahJam10', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
