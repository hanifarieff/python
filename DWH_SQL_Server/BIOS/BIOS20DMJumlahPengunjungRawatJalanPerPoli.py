...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS20DMJumlahPengunjungRawatJalanPerPoli.txt","w")
t0 = time.time()
...
source = pd.read_sql_query(""" SELECT 
                                    ...
                                    NamaPoli,
                                    Jumlah 
                                FROM dwhrscm_talend.[20DMJumlahPengunjungRawatJalanPerPoli]
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,NamaPoli,Jumlah FROM dwhrscm_talend.BIOS20DMJumlahPengunjungRawatJalanPerPoli WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS20DMJumlahPengunjungRawatJalanPerPoli', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
