...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS24DMJumlahPasienRawatInap.txt","w")
...
source = pd.read_sql_query(""" SELECT 
                                    ...
                                    KodeKelas,
                                    Jumlah 
                                FROM dwhrscm_talend.[38DMJumlahPasienRawatInap]
                                WHERE TglTransaksi >= CAST(DATEADD(DAY, -15, GETDATE()) as date)
                                AND TglTransaksi <= CAST(GETDATE() as date)
                                ORDER BY TglTransaksi,KodeKelas
                            """, conn_dwh_sqlserver)
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f"SELECT TglTransaksi,KodeKelas,Jumlah FROM dwhrscm_talend.BIOS24DMJumlahPasienRawatInap WHERE TglTransaksi IN ('{tgltransaksi}') ORDER BY TglTransaksi"
    ...
    modified = changes[changes[['TglTransaksi','KodeKelas']].apply(tuple,1).isin(target[['TglTransaksi','KodeKelas']].apply(tuple,1))]
    ...
    inserted = changes[~changes[['TglTransaksi','KodeKelas']].apply(tuple,1).isin(target[['TglTransaksi','KodeKelas']].apply(tuple,1))]
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS24DMJumlahPasienRawatInap', schema='dwhrscm_talend',con=conn_bios_sqlserver, if_exists='append',index=False)
        ...
        try:
            #update data
            updated_to_sql(modified, 'BIOS24DMJumlahPasienRawatInap', 'TglTransaksi','KodeKelas')
