...
# bikin log ke file
sys.stdout = open("C:/TestPython/DWH_SQL_Server/BIOS/logs/LogBIOS38DMJumlahLayananForensik.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.[113DMJumlahLayananForensik]
...
query = f'SELECT TglTransaksi,Jumlah FROM dwhrscm_talend.BIOS38DMJumlahLayananForensik WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent']=0
    inserted.to_sql('BIOS38DMJumlahLayananForensik',schema='dwhrscm_talend', con = conn_bios_sqlserver, if_exists = 'append', index=False)
    ...
    try:
        # update data
        updated_to_sql(modified, 'BIOS38DMJumlahLayananForensik', 'TglTransaksi')
