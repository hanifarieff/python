...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS41DMJumlahPasienRawatDarurat.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,Jumlah FROM 86DMJumlahPasienRawatDarurat
                                WHERE TglTransaksi >= DATE_FORMAT(DATE_SUB(now(), interval 4 DAY), "%%Y-%%m-%%d 00:00:00") 
                                AND TglTransaksi <= DATE_FORMAT(DATE_SUB(now(),interval 1 day), "%%Y-%%m-%%d 23:59:59")
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS41DMJumlahPasienRawatDarurat WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS41DMJumlahPasienRawatDarurat', con=conn_dwh_mysql, if_exists='append',index=False)
    ...
    try:
        #update data
        updated_to_sql(modified, 'BIOS41DMJumlahPasienRawatDarurat', 'TglTransaksi')
