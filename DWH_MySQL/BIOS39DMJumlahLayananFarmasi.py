...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS39DMJumlahLayananFarmasi.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,Jumlah FROM 88DMJumlahLayananFarmasi
                            ...
                            """, conn_dwh_mysql)
# try:
#     source.to_sql('BIOS39DMJumlahLayananFarmasi', con=conn_dwh_mysql, if_exists='append',index=False)
...
source['Jumlah']=source['Jumlah'].apply(np.int64)
tgltransaksi = tuple(source["TglTransaksi"].unique())
print(tgltransaksi)
print(source)

query = f'SELECT TglTransaksi,Jumlah FROM BIOS39DMJumlahLayananFarmasi WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent']=0
    inserted.to_sql('BIOS39DMJumlahLayananFarmasi', con = conn_dwh_mysql, if_exists = 'append', index=False)
    ...
    try:
        # update data
        updated_to_sql(modified, 'BIOS39DMJumlahLayananFarmasi', 'TglTransaksi')
