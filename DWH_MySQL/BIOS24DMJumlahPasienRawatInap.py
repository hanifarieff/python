...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS24DMJumlahPasienRawatInap.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,KodeKelas, Jumlah FROM 24DMJumlahPasienRawatInap
...
query = f'SELECT TglTransaksi,KodeKelas,Jumlah FROM BIOS24DMJumlahPasienRawatInap WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS24DMJumlahPasienRawatInap', con=conn_dwh_mysql, if_exists='append',index=False)
