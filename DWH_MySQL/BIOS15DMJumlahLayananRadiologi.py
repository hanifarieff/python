...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS15DMJumlahLayananRadiologi.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 15DMJumlahLayananRadiologi                               
...
if source.empty:
    ...
    if len(source) == 1:        
        ...
        query = f'SELECT TglTransaksi,Jumlah FROM BIOS15DMJumlahLayananRadiologi WHERE TglTransaksi IN ({tgltransaksi}) ORDER BY TglTransaksi'
    ...
    if modified.empty:
        inserted['IsSent'] = 0
        inserted.to_sql('BIOS15DMJumlahLayananRadiologi', con=conn_dwh_mysql, if_exists='append',index=False)
        ...
        try:
            #update data
            updated_to_sql(modified, 'BIOS15DMJumlahLayananRadiologi', 'TglTransaksi')
