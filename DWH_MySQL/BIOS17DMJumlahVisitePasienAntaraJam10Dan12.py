...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS17DMJumlahVisitePasienAntaraJam10Dan12.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 17DMJumlahVisitePasienAntaraJam10Dan12
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS17DMJumlahVisitePasienAntaraJam10Dan12 WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS17DMJumlahVisitePasienAntaraJam10Dan12', con=conn_dwh_mysql, if_exists='append',index=False)
