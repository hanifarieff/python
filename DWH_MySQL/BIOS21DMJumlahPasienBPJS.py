...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS21DMJumlahPasienBPJS.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 21DMJumlahPasienBPJS
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS21DMJumlahPasienBPJS WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS21DMJumlahPasienBPJS', con=conn_dwh_mysql, if_exists='append',index=False)
