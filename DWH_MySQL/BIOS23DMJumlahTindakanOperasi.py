...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS23DMJumlahTindakanOperasi.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 23DMJumlahTindakanOperasi
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS23DMJumlahTindakanOperasi WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS23DMJumlahTindakanOperasi', con=conn_dwh_mysql, if_exists='append',index=False)
    ...
    try:
        #update data
        updated_to_sql(modified, 'BIOS23DMJumlahTindakanOperasi', 'TglTransaksi')
