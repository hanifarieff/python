...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS14DMJumlahLayananLaboratorium2.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,NamaLayanan, Jumlah FROM 14DMJumlahLayananLaboratorium2
...
query = f'SELECT TglTransaksi,NamaLayanan,Jumlah FROM BIOS14DMJumlahLayananLaboratorium2 WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS14DMJumlahLayananLaboratorium2', con=conn_dwh_mysql, if_exists='append',index=False)
