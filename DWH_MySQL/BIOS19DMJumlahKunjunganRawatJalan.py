...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS19DMJumlahKunjunganRawatJalan.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 19DMJumlahKunjunganRawatJalan
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS19DMJumlahKunjunganRawatJalan WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS19DMJumlahKunjunganRawatJalan', con=conn_dwh_mysql, if_exists='append',index=False)
