...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS30DMJumlahKegiatanVisitePasienPertama.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 30DMJumlahKegiatanVisitePasienPertama
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS30DMJumlahKegiatanVisitePasienPertama WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS30DMJumlahKegiatanVisitePasienPertama', con=conn_dwh_mysql, if_exists='append',index=False)
