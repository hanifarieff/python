...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS22DMJumlahPasienNonBPJS.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 22DMJumlahPasienNonBPJS
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS22DMJumlahPasienNonBPJS WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS22DMJumlahPasienNonBPJS', con=conn_dwh_mysql, if_exists='append',index=False)
