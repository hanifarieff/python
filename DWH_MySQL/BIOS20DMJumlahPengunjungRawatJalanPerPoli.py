...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS20DMJumlahPengunjungRawatJalanPerPoli.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, NamaPoli,Jumlah FROM 20DMJumlahPengunjungRawatJalanPerPoli
                                
                                WHERE TglTransaksi >= DATE_SUB(DATE(now()), interval 4 DAY)
...
query = f'SELECT TglTransaksi,NamaPoli,Jumlah FROM BIOS20DMJumlahPengunjungRawatJalanPerPoli WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
modified = changes[changes[['TglTransaksi','NamaPoli']].apply(tuple,1).isin(target[['TglTransaksi','NamaPoli']].apply(tuple,1))]
...
inserted = changes[~changes[['TglTransaksi','NamaPoli']].apply(tuple,1).isin(target[['TglTransaksi','NamaPoli']].apply(tuple,1))]
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS20DMJumlahPengunjungRawatJalanPerPoli', con=conn_dwh_mysql, if_exists='append',index=False)
    ...
    try:
        #update data
        updated_to_sql(modified, 'BIOS20DMJumlahPengunjungRawatJalanPerPoli', 'TglTransaksi','NamaPoli')
