...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS18DMJumlahVisitePasienDiatasJam12.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi, Jumlah FROM 18DMJumlahVisitePasienDiatasJam12
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS18DMJumlahVisitePasienDiatasJam12 WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS18DMJumlahVisitePasienDiatasJam12', con=conn_dwh_mysql, if_exists='append',index=False)
