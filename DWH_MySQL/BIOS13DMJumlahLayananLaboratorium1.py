...
sys.stdout = open("C:/TestPython/DWH_MySQL/logs/LogBIOS13DMJumlahLayananLaboratorium1.txt","w")
...
source = pd.read_sql_query(""" SELECT TglTransaksi,Jumlah FROM 13DMJumlahLayananLaboratorium1
                                -- WHERE TglTransaksi >= '2023-03-01 00:00:00' AND TglTransaksi <= '2023-03-05 23:59:59'
...
query = f'SELECT TglTransaksi,Jumlah FROM BIOS13DMJumlahLayananLaboratorium1 WHERE TglTransaksi IN {tgltransaksi} ORDER BY TglTransaksi'
...
if modified.empty:
    inserted['IsSent'] = 0
    inserted.to_sql('BIOS13DMJumlahLayananLaboratorium1', con=conn_dwh_mysql, if_exists='append',index=False)
    ...
    try:
        #update data
        updated_to_sql(modified, 'BIOS13DMJumlahLayananLaboratorium1', 'TglTransaksi')
